...
for i in range(4):#npts):
  rr.append(random.randint(0,npts-1))
  rr2.append(random.randint(1,npts))
rr.sort()
rr2.sort()
print(rr)
print(rr2)
...
y1=0.0
for i in range(4):#npts):
  fname=fname+str(rr[i])+str(rr2[i])
  rad1=((1.0/float(npts))*float(rr[i]+1))*size
  rad2=((1.0/float(npts))*float(rr2[i]+1))*(size*(2.0/4.0))
  x2=math.cos(ang1)*rad2
  y2=math.sin(ang1)*rad2
  draw.append([[x1,y1],[x2,y2]])
  x1=0.0
  y1=rad1
  draw.append([[x1,y1],[x2,y2]])
draw.append([[0.0,size],[x2,y2]])
